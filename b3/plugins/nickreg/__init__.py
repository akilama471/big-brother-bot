...
class NickregPlugin(b311.plugin.Plugin):
    adminPlugin = None
    crontab = None
    ignore_till = 0
    ...
    def onLoadConfig(self):
        ...
        """
        self.min_level = self.getSetting('settings', 'min_level', b311.LEVEL, 20)
        self.min_level_global_manage = self.getSetting('settings', 'min_level_global_manage', b311.LEVEL, 100,
                                                       lambda x: clamp(x, minv=self.min_level))
        self.max_nicks = self.getSetting('settings', 'max_nicks', b311.INTEGER, 3)
        self.interval = self.getSetting('settings', 'interval', b311.INTEGER, 30)
    ...
    def onStartup(self):
        ...
        if 'nicks' not in self.console.storage.getTables():
            sql_path_main = b311.getAbsolutePath('@b311/plugins/nickreg/sql')
        ...
        # install crontab
        self.crontab = b311.cron.PluginCronTab(self, self.execute_crontab, '*/%s' % self.interval)
    ...
    def _process_name(self, name):
        ...
        """
        return b311.functions.escape(self.console.stripColors(name).lower(), "'")
    ...
    def warn_client_for_nick_steal(self, client):
        ...
        """
        self.adminPlugin.warnClient(client, 'You are using a registered nickname, please change it!', None, False, '',
                                    '10m')
    ...
    def check_client_for_nick_steal(self, client):
        ...
        if client and client.id and client.pbid not in ('WORLD', 'Server'):
            ...
            cursor = self.console.storage.query(
                """SELECT * FROM nicks WHERE name LIKE '%s'""" % self._process_name(client.name))
            if cursor.EOF:
                ...
                if int(row['client_id']) != int(client.id):
                    self.debug('warning client @%s for nickname stealing (%s): owner is client @%s', client.id,
                               client.name, row['client_id'])
                    self.warn_client_for_nick_steal(client)
                else:
                    self.debug('client @%s is the owner of registered nickname (%s): everything ok', client.id,
                               client.name)
    ...
    def cmd_listnick(self, data, client, cmd=None):
        ...
        if sclient.id != client.id and (
                client.maxLevel < self.min_level_global_manage or sclient.maxLevel > client.maxLevel):
    ...
    def cmd_regnick(self, data, client, cmd=None):
        ...
        cursor.close()
        cursor = self.console.storage.query(
            """SELECT COUNT(*) AS num_registered FROM nicks WHERE client_id = %s""" % client.id)
        ...
        cursor.close()
        self.console.storage.query("""INSERT INTO nicks (client_id, name) VALUES ('%s', '%s')""" % (
        client.id, self._process_name(client.name)))
