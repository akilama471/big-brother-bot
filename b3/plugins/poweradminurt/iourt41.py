...
import b311.events
import b311.plugin
from b311.functions import clamp
from b311.functions import getCmd
from . import __author__
from . import __version__


class Poweradminurt41Plugin(b311.plugin.Plugin):
    ...
    def loadNameChecker(self):
        ...
        """
        self._ninterval = self.getSetting('namechecker', 'ninterval', b311.INT, self._ninterval,
                                          lambda x: clamp(x, maxv=59))
        self._checkdupes = self.getSetting('namechecker', 'checkdupes', b311.BOOL, self._checkdupes)
        self._checkunknown = self.getSetting('namechecker', 'checkunknown', b311.BOOL, self._checkunknown)
        self._checkbadnames = self.getSetting('namechecker', 'checkbadnames', b311.BOOL, self._checkbadnames)
        self._checkchanges = self.getSetting('namechecker', 'checkchanges', b311.BOOL, self._checkchanges)
        self._checkallowedchanges = self.getSetting('namechecker', 'checkallowedchanges', b311.INT,
    ...
    def loadTeamBalancer(self):
        ...
        """
        self._tinterval = self.getSetting('teambalancer', 'tinterval', b311.INT, self._tinterval,
                                          lambda x: clamp(x, maxv=59))
        self._teamdiff = self.getSetting('teambalancer', 'teamdifference', b311.INT, self._teamdiff,
                                         lambda x: clamp(x, minv=1, maxv=9))
        self._tmaxlevel = self.getSetting('teambalancer', 'maxlevel', b311.LEVEL, self._tmaxlevel)
        self._announce = self.getSetting('teambalancer', 'announce', b311.INT, self._announce)
        ...
        self._team_change_force_balance_enable = self.getSetting('teambalancer', 'team_change_force_balance_enable',
                                                                 b311.BOOL, self._team_change_force_balance_enable)
        # 10/22/2008 - 1.4.0b10 - mindriot
        self._autobalance_gametypes = self.getSetting('teambalancer', 'autobalance_gametypes', b311.STR,
                                                      self._autobalance_gametypes, lambda x: x.lower())
        self._autobalance_gametypes_array = re.split(r'[\s,]+', self._autobalance_gametypes)
        self._teamLocksPermanent = self.getSetting('teambalancer', 'teamLocksPermanent', b311.BOOL,
                                                   self._teamLocksPermanent)
        self._ignorePlus = self.getSetting('teambalancer', 'timedelay', b311.INT, self._ignorePlus)

    def loadSkillBalancer(self):
        """
        Setup the skill balancer
        """
        self._skinterval = self.getSetting('skillbalancer', 'interval', b311.INT, self._skinterval,
                                           lambda x: clamp(x, maxv=59))
        self._skilldiff = self.getSetting('skillbalancer', 'difference', b311.FLOAT, self._skilldiff,
                                          lambda x: clamp(x, minv=0.1, maxv=9.0))
        self._skill_balance_mode = self.getSetting('skillbalancer', 'mode', b311.INT, self._skill_balance_mode)
        self._minbalinterval = self.getSetting('skillbalancer', 'min_bal_interval', b311.INT, self._minbalinterval)

    def loadVoteDelayer(self):
        """
        Setup the vote delayer
        """
        self._votedelay = self.getSetting('votedelay', 'votedelay', b311.INT, self._votedelay)
    ...
    def loadSpecChecker(self):
        ...
        """
        self._sinterval = self.getSetting('speccheck', 'sinterval', b311.INT, self._sinterval,
                                          lambda x: clamp(x, maxv=59))
        self._smaxspectime = self.getSetting('speccheck', 'maxspectime', b311.INT, self._smaxspectime)
        self._smaxlevel = self.getSetting('speccheck', 'maxlevel', b311.LEVEL, self._smaxlevel)
        ...
        self._smaxplayers = self.getSetting('speccheck', 'maxplayers', b311.INT, smaxplayers)

    def loadMoonMode(self):
        """
        Setup the moon mode
        """
        self._moon_on_gravity = self.getSetting('moonmode', 'gravity_on', b311.INT, self._moon_on_gravity)
        self._moon_off_gravity = self.getSetting('moonmode', 'gravity_off', b311.INT, self._moon_off_gravity)

    def loadPublicMode(self):
        """
        Setup the public mode
        """
        self._randnum = self.getSetting('publicmode', 'randnum', b311.INT, self._randnum)

        try:

            padic = self.getSetting('publicmode', 'usedic', b311.BOOL, False)
            if padic:
        ...
        self._papublic_password = self.getSetting('publicmode', 'g_password', b311.STR, None)
    ...
    def loadMatchMode(self):
        ...
        """
        self._match_plugin_disable = self.getSetting('matchmode', 'plugins_disable', b311.LIST, [])
        ...
        except (b311.config.NoSectionError, b311.config.NoOptionError, KeyError), e:
    ...
    def loadBotSupport(self):
        ...
        """
        self._botenable = self.getSetting('botsupport', 'bot_enable', b311.BOOL, self._botenable)
        self._botskill = self.getSetting('botsupport', 'bot_skill', b311.INT, self._botskill,
                                         lambda x: clamp(x, minv=1, maxv=5))
        self._botminplayers = self.getSetting('botsupport', 'bot_minplayers', b311.INT, self._botminplayers,
                                              lambda x: clamp(x, minv=0, maxv=16))
        self._botmaps = self.getSetting('botsupport', 'bot_maps', b311.LIST, [])
    ...
    def loadHeadshotCounter(self):
        ...
        self._hsenable = self.getSetting('headshotcounter', 'hs_enable', b311.BOOL, self._hsenable)
        self._hsresetvars = self.getSetting('headshotcounter', 'reset_vars', b311.STR, self._hsresetvars,
                                            validate_reset_vars)
        self._hsbroadcast = self.getSetting('headshotcounter', 'broadcast', b311.BOOL, self._hsbroadcast)
        self._hsall = self.getSetting('headshotcounter', 'announce_all', b311.BOOL, self._hsall)
        self._hspercent = self.getSetting('headshotcounter', 'announce_percentages', b311.BOOL, self._hspercent)
        self._hspercentmin = self.getSetting('headshotcounter', 'percent_min', b311.INT, self._hspercentmin)
        self._hswarnhelmet = self.getSetting('headshotcounter', 'warn_helmet', b311.BOOL, self._hswarnhelmet)
        self._hswarnhelmetnr = self.getSetting('headshotcounter', 'warn_helmet_nr', b311.INT, self._hswarnhelmetnr)
        self._hswarnkevlar = self.getSetting('headshotcounter', 'warn_kevlar', b311.BOOL, self._hswarnkevlar)
        self._hswarnkevlarnr = self.getSetting('headshotcounter', 'warn_kevlar_nr', b311.INT, self._hswarnkevlarnr)
    ...
    def loadRotationManager(self):
        ...
        """
        self._rmenable = self.getSetting('rotationmanager', 'rm_enable', b311.BOOL, self._rmenable)
        if self._rmenable:
            self._switchcount1 = self.getSetting('rotationmanager', 'switchcount1', b311.INT, self._switchcount1)
            self._switchcount2 = self.getSetting('rotationmanager', 'switchcount2', b311.INT, self._switchcount2)
            self._hysteresis = self.getSetting('rotationmanager', 'hysteresis', b311.INT, self._hysteresis)
            self._rotation_small = self.getSetting('rotationmanager', 'smallrotation', b311.STR, self._rotation_small)
            self._rotation_medium = self.getSetting('rotationmanager', 'mediumrotation', b311.STR,
                                                    self._rotation_medium)
            self._rotation_large = self.getSetting('rotationmanager', 'largerotation', b311.STR, self._rotation_large)
            self._gamepath = self.getSetting('rotationmanager', 'gamepath', b311.STR, self._gamepath)
        else:
    ...
    def loadSpecial(self):
        ...
        """
        self._slapSafeLevel = self.getSetting('special', 'slap_safe_level', b311.LEVEL, self._slapSafeLevel)
        self._full_ident_level = self.getSetting('special', 'paident_full_level', b311.LEVEL, self._full_ident_level)
    ...
    def installCrontabs(self):
        ...
        if self._ninterval > 0:
            self._ncronTab = b311.cron.PluginCronTab(self, self.namecheck, 0, '*/%s' % self._ninterval)
            self.console.cron + self._ncronTab
        if self._tinterval > 0:
            self._tcronTab = b311.cron.PluginCronTab(self, self.teamcheck, 0, '*/%s' % self._tinterval)
            self.console.cron + self._tcronTab
        if self._sinterval > 0:
            self._scronTab = b311.cron.PluginCronTab(self, self.speccheck, 0, '*/%s' % self._sinterval)
            self.console.cron + self._scronTab
        if self._skinterval > 0:
            self._skcronTab = b311.cron.PluginCronTab(self, self.skillcheck, 0, '*/%s' % self._skinterval)
            self.console.cron + self._skcronTab
    ...
    def _getRandomTeams(self, clients, checkforced=False):
        ...
        for c in clients:
            # ignore spectators
            if c.team in (b311.TEAM_BLUE, b311.TEAM_RED):
                if checkforced and c.isvar(self, 'paforced'):
                    if c.team == b311.TEAM_BLUE:
                        blue.append(c)
                    else:
                        red.append(c)
                else:
    ...
    def _getTeamScoreDiffForAdvise(self, minplayers=None):
        ...
        blue = [c for c in clients if c.team == b311.TEAM_BLUE]
        red = [c for c in clients if c.team == b311.TEAM_RED]
    ...
    def _move(self, blue, red, scores=None):
        ...
        blue = [c for c in blue if c.team != b311.TEAM_BLUE]
        red = [c for c in red if c.team != b311.TEAM_RED]
        ...
        clients = self.console.clients.getList()
        numblue = len([c for c in clients if c.team == b311.TEAM_BLUE])
        numred = len([c for c in clients if c.team == b311.TEAM_RED])
    ...
    def _randTeams(self, times, slack, maxmovesperc=None):
        ...
        oldblue = [c for c in clients if c.team == b311.TEAM_BLUE]
        oldred = [c for c in clients if c.team == b311.TEAM_RED]
    ...
    def cmd_paunskuffle(self, data, client, cmd=None):
        ...
        clients = self.console.clients.getList()
        scores = self._getScores(clients)
        decorated = [(scores.get(c.id, 0), c) for c in clients if c.team in (b311.TEAM_BLUE, b311.TEAM_RED)]
        decorated.sort()
    ...
    def cmd_paskuffle(self, data=None, client=None, cmd=None):
        ...
        if client:
            if (client.team == b311.TEAM_BLUE and client.cid not in [c.cid for c in blue]) or \
                    (client.team == b311.TEAM_RED and client.cid not in [c.cid for c in red]):
    ...
    def onTeamChange(self, event):
        ...
        if not self._matchmode and client.isvar(self, 'paforced'):
            ...
            if team != b311.TEAM_UNKNOWN and team != self.console.getTeam(forcedteam):
        ...
        if self._team_change_force_balance_enable and not self._matchmode:
            ...
            if self.isEnabled() and not self._balancing:
                ...
                if team == b311.TEAM_SPEC:
                    self.verbose('player joined specs')
                    # done balancing
                    self._balancing = False
                    return None
                elif team == b311.TEAM_UNKNOWN:
    ...
    def teambalance(self):
        ...
        if self.isEnabled() and not self._balancing and not self._matchmode:
            ...
            if abs(self._teamred - self._teamblue) <= self._teamdiff:
                ...
                if self._teamred > self._teamblue:
                    newteam = 'blue'
                    oldteam = b311.TEAM_RED
                else:
                    newteam = 'red'
                    oldteam = b311.TEAM_BLUE
        ...
        if self.isEnabled() and self._g_maxGameClients == 0 and not self._matchmode:
            ...
            for c in clients:
                ...
                if c.maxLevel >= self._smaxlevel:
                    ...
                    continue
                elif c.team == b311.TEAM_SPEC and (self.console.time() - c.var(self, 'teamtime').value) > \
