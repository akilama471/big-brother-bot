...
from b311.functions import levenshteinDistance
from b311.functions import soundex
from b311.parsers.bf3 import GAME_MODES_NAMES
from b311.parsers.bf3 import GUNMASTER_WEAPONS_PRESET_BY_INDEX
...
from b311.plugin import Plugin
        ...
        if not data:
            ...
            if len(clients) == 1:
                ...
                if sclient.id != client.id and sclient.maxLevel > client.maxLevel:
                    if sclient.maxGroup:
                        client.message(self.getMessage('operation_denied_level',
                                                       {'name': sclient.name, 'group': sclient.maxGroup.name}))
                    else:
...
class Poweradminbf3Plugin(Plugin, vip_commands_mixin):
                ...
                if not sclient:
                    ...
                    try:
                        ...
                        cmd.sayLoudOrPM(client,
                                        '%s forced from team %s to team %s' % (sclient.cid, original_team, newteam))
                    except CommandFailedError, err:
        ...
        if client.maxLevel < self.no_level_check_level:
            ...
            if client.maxLevel < sclientA.maxLevel:
                if sclientA.maxGroup:
                    client.message(self.getMessage('operation_denied_level',
                                                   {'name': sclientA.name, 'group': sclientA.maxGroup.name}))
                else:
                    client.message(self.getMessage('operation_denied'))
                return
            if client.maxLevel < sclientB.maxLevel:
                if sclientB.maxGroup:
                    client.message(self.getMessage('operation_denied_level',
                                                   {'name': sclientB.name, 'group': sclientB.maxGroup.name}))
                else:
                    client.message(self.getMessage('operation_denied'))
                return
    ...
    def cmd_setnextmap(self, data, client=None, cmd=None):
        ...
        if not len(maplist):
            ...
            if not len(matching_maps):
                ...
                self.console.write(('mapList.setNextMapIndex', next_map_index))
        if client:
            map_info = '%s (%s) %s round%s' % (
            self.console.getEasyName(map_id), self.console.getGameMode(gamemode_id), num_rounds,
    ...
    def cmd_loadconfig(self, data, client=None, cmd=None):
        ...
        if not data:
            ...
            if not len(found_names):
                ...
                try:
                    self._load_server_config_from_file(client, config_name=found_names[0], file_path=_fName,
                                                       threaded=True)
    ...
    def cmd_idle(self, data, client=None, cmd=None):
        ...
        if not data:
            ...
            if data.lower() not in expected_values and minutes is None:
                client.message("unexpected value '%s'. Available modes : %s or a number of minutes" % (
                data, ', '.join(expected_values)))
                return
    ...
    def cmd_yellteam(self, data, client, cmd=None):
        ...
        if not data:
            ...
            self.console.write(self.console.getCommand('yellTeam', message=data, teamId=client.teamId,
                                                       big_msg_duration=self._yell_duration))
    ...
    def cmd_yellsquad(self, data, client, cmd=None):
        ...
        if not data:
            ...
            self.console.write(
                self.console.getCommand('yellSquad', message=data, teamId=client.teamId, squadId=client.squad,
                                        big_msg_duration=self._yell_duration))
    ...
    def cmd_yellplayer(self, data, client, cmd=None):
        ...
        if sclient:
            self.console.write(self.console.getCommand('bigmessage', message=message, cid=sclient.cid,
                                                       big_msg_duration=self._yell_duration))
    ...
    def _load_scrambler(self):
        ...
        try:
            try:
                ...
                if len(invalid_gamemodes):
                    self.warning(
                        r"option 'srambler\gamemodes_blacklist' in your config file has invalid gamemode(s) : %s" % ', '.join(
                            invalid_gamemodes))
        ...
        self.info('auto scrambler will ignore gamemodes : %s' % ', '.join(blacklist) if len(
            blacklist) else 'auto scrambler will not ignore any gamemodes')
    ...
    def _load_config_path(self):
        ...
        if self._configPath is None:
            self.error(
                'Could not find the server config files directory. Update your config file with an existing directory name in option "config_path" of section "preferences".')
        else:
    ...
    def _load_configmanager_config_path(self):
        ...
        if self._configManager_configPath is None:
            self.error(
                'Could not find the configmanager directory. Create a directory named \'configmanager\' next to your plugin config file')
        else:
            self.info("ConfigManager directory : %s" % self._configManager_configPath)

    def _load_no_level_check_level(self):
        self.no_level_check_level = self.getSetting('preferences', 'no_level_check_level', b311.LEVEL, default=100)
        self.info('no_level_check_level is %s' % self.no_level_check_level)

    def _load_autobalance_settings(self):
        self._no_autoassign_level = self.getSetting('preferences', 'no_autoassign_level', value_type=b311.LEVEL,
                                                    default=20)
        ...
        try:
            ...
            self.debug(err)
            self.warning(
                'Could not read level value from config option \"preferences\\autoassign\". Using default value \"%s\" instead. (%s)' % (
                self._autoassign, err))
        except Exception as err:
            self.error(err)
        ...
        try:
            ...
            self.info(
                'No config option \"preferences\\autobalance\" found. Using default value : %s' % self._autobalance)
        except ValueError, err:
            self.debug(err)
            self.warning(
                'Could not read level value from config option \"preferences\\autobalance\". Using default value \"%s\" instead. (%s)' % (
                self._autobalance, err))
        except Exception as err:
            self.error(err)
        ...
        try:
            ...
            self.info(
                'No config option \"preferences\\autobalance_timer\" found. Using default value : %s' % self._autobalance_timer)
        except ValueError, err:
            self.debug(err)
            self.warning(
                'Could not read level value from config option \"preferences\\autobalance_timer\". Using default value \"%s\" instead. (%s)' % (
                self._autobalance_timer, err))
        except Exception as err:
            self.error(err)
        ...
        try:
            ...
            self.info(
                r'No config option "preferences\team_swap_threshold" found. Using default value : %s' % self._team_swap_threshold)
        except ValueError, err:
            self.debug(err)
            self.warning(
                r'Could not read level value from config option "preferences\team_swap_threshold". Using default value "%s" instead. (%s)' % (
                self._team_swap_threshold, err))
        except Exception as err:
            self.error(err)
        ...
        try:
            ...
            self.info(
                'No config option \"preferences\\team_swap_threshold_prop\" found. Using default value : %s' % self._team_swap_threshold_prop)
        except ValueError, err:
            self.debug(err)
            self.warning(
                'Could not read level value from config option \"preferences\\team_swap_threshold_prop\". Using default value \"%s\" instead. (%s)' % (
                self._team_swap_threshold_prop, err))
        except Exception as err:
            self.error(err)
    ...
    def _load_yell_duration(self):
        try:
            ...
            self.info(
                r'No config option "preferences\yell_duration" found. Using default value : %s' % self._yell_duration)
        except ValueError, err:
            self.debug(err)
            self.warning(
                r'Could not read value from config option "preferences\yell_duration". Using default value "%s" instead. (%s)' % (
                self._yell_duration, err))
        except Exception as err:
            self.error(err)
    ...
    def _get_server_config_directory(dir_name=''):
        return b311.getAbsolutePath('@b311/plugins/poweradminbf3/%s' % dir_name)
        ...
        for line_index, line, m in cvars_matches:
            if m.group('value'):
                ...
                try:
                    ...
                    self._sendMessage(client, 'Error "%s" received at line %s when sending "%s" to server' % (
                    err.message, line_index, line))
            else:
                # read cvar
                try:
                    ...
                    self._sendMessage(client, 'Error "%s" received at line %s when sending "%s" to server' % (
                    err.message, line_index, m.group('cvar')))
            ...
            for line_index, line, m in map_item_matches:
                try:
                    ...
                    self._sendMessage(client,
                                      "Error adding map \"%s\" on line %s : %s" % (line, line_index, err.message))
            try:
                self.console.write((
                                   'mapList.save',))  # write current in-memory map list to server config file so if the server restarts our list is recovered.
    ...
    def config_manager_check_config(self):
        ...
        """
        if os.path.isfile(
                self._configManager_configPath + os.path.sep + self._next_typeandmap + '.cfg'):  # b3_<gametype>_<mapname>.cfg
            ...
            self._load_server_config_from_file(client=None, config_name=self._next_typeandmap, file_path=_fName,
                                               threaded=True)

        elif os.path.isfile(
                self._configManager_configPath + os.path.sep + self._next_gametype + '.cfg'):  # b3_<gametype>.cfg
            _fName = self._configManager_configPath + os.path.sep + self._next_gametype + '.cfg'
            self.info('Executing %s.cfg' % self._next_gametype)
            self._load_server_config_from_file(client=None, config_name=self._next_gametype, file_path=_fName,
                                               threaded=True)
    ...
    def start_autobalance_cron(self):
        ...
        if self._autobalance:
            ...
            self._cronTab_autobalance = b311.cron.OneTimeCronTab(self.run_autobalance, second=s, minute=m)
            self.console.cron + self._cronTab_autobalance
