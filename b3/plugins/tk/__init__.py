...
class TkPlugin(b311.plugin.Plugin):
    ...
    def onLoadConfig(self):
        ...
        """
        self._issue_warning = self.getSetting('settings', 'issue_warning', b311.STR, self._issue_warning)
        self._round_grace = self.getSetting('settings', 'round_grace', b311.INT, self._round_grace)
        self._maxPoints = self.getSetting('settings', 'max_points', b311.INT, self._maxPoints)
        self._private_messages = self.getSetting('settings', 'private_messages', b311.BOOL, self._private_messages)
        self._damage_threshold = self.getSetting('settings', 'damage_threshold', b311.INT, self._damage_threshold)
        self._tk_warn_duration = self.getSetting('settings', 'warn_duration', b311.STR, self._tk_warn_duration)
        self._warn_level = self.getSetting('settings', 'warn_level', b311.INT, self._warn_level)
        self._tkpointsHalflife = self.getSetting('settings', 'halflife', b311.INT, self._tkpointsHalflife)
        self._grudge_enable = self.getSetting('settings', 'grudge_enable', b311.BOOL, self._grudge_enable)
        self._grudge_level = self.getSetting('settings', 'grudge_level', b311.INT, self._grudge_level)
    ...
    def onStartup(self):
        ...
        if self._tkpointsHalflife > 0:
            ...
            self._cronTab_tkhalflife = b311.cron.OneTimeCronTab(self.halveTKPoints, second=sec, minute=minute)
    ...
    def onEvent(self, event):
        ...
        if self.console.game.gameType in self._ffa:
            ...
            if self._tkpointsHalflife > 0:
                ...
                self._cronTab_tkhalflife = b311.cron.OneTimeCronTab(self.halveTKPoints, second=s, minute=m)
                self.console.cron + self._cronTab_tkhalflife
                self.debug('TK crontab started')
