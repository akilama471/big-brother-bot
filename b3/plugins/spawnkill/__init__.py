...
class SpawnkillPlugin(b311.plugin.Plugin):
    adminPlugin = None
    ...
    def onLoadConfig(self):
        ...
        for index in ('hit', 'kill'):
            self.settings[index]['maxlevel'] = self.getSetting(index, 'maxlevel', b311.LEVEL,
                                                               self.settings[index]['maxlevel'])
            self.settings[index]['delay'] = self.getSetting(index, 'delay', b311.INT, self.settings[index]['delay'])
            self.settings[index]['duration'] = self.getSetting(index, 'duration', b311.DURATION,
                                                               self.settings[index]['duration'])
            self.settings[index]['reason'] = self.getSetting(index, 'reason', b311.STR, self.settings[index]['reason'])
            self.settings[index]['penalty'] = self.getSetting(index, 'penalty', b311.STR,
                                                              self.settings[index]['penalty'], validate)
