...
class TranslatorPlugin(b311.plugin.Plugin):
    adminPlugin = None
    cmdPrefix = None
    ...
    def onLoadConfig(self):
        ...
        self.settings['default_source_language'] = self.getSetting('settings', 'default_source_language', b311.STR,
                                                                   'auto', validate_source)
        self.settings['default_target_language'] = self.getSetting('settings', 'default_target_language', b311.STR,
                                                                   'en', validate_target)
        self.settings['display_translator_name'] = self.getSetting('settings', 'display_translator_name', b311.BOOL,
                                                                   True)
        self.settings['translator_name'] = self.getSetting('settings', 'translator_name', b311.STR, '^7[^1T^7]')
        self.settings['min_sentence_length'] = self.getSetting('settings', 'min_sentence_length', b311.INT, 6)
        self.settings['microsoft_client_id'] = self.getSetting('settings', 'microsoft_client_id', b311.STR)
        self.settings['microsoft_client_secret'] = self.getSetting('settings', 'microsoft_client_secret', b311.STR)
        ...
        if '*' in data:
            ...
            if source and source != 'auto':
                if source not in self.languages:
                    self.verbose(
                        'invalid source language (%s) specified in !translate command: unable to translate' % source)
            ...
            if target:
                if target not in self.languages:
                    self.verbose(
                        'invalid target language (%s) specified in !translate command: unable to translate' % target)
