...
class DuelPlugin(b311.plugin.Plugin):
    adminPlugin = None
...
class Duel(object):
    ...
    def __init__(self, clientA, clientB):
        ...
        """
        if not isinstance(clientA, b311.clients.Client):
            raise DuelError('clientA is not a client')
        if not isinstance(clientB, b311.clients.Client):
    ...
    def registerKillEvent(self, event):
        ...
        if self.status == Duel.STATUS_STARTED:
            ...
            if not isinstance(event, b311.events.Event) or \
    ...
    def announceScoreTo(self, client):
        ...
        :param client: b311.client.Client instance
        """
        if self.status == Duel.STATUS_STARTED:
