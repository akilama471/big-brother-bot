...
class PluginmanagerPlugin(b311.plugin.Plugin):
    _adminPlugin = None
    ...
    def do_load(self, client, name=None):
        ...
        def _get_plugin_config(p_name, p_clazz):
            ...
            def _search_config_file(match):
                ...
                search = '%s%s*%s*' % (b311.getAbsolutePath('@b311\\conf'), os.path.sep, match)
                ...
                search = '%s%s*%s*' % (
                os.path.join(b311.getAbsolutePath(extplugins_dir), match, 'conf'), os.path.sep, match)
                self.debug('searching for configuration file(s) matching: %s' % search)
                collection = glob.glob(search)
                return collection
            ...
            if len(search_path) == 0:
                if p_clazz.requiresConfigFile:
                    raise b311.config.ConfigFileNotFound('could not find any configuration file')
        ...
        try:
            ...
            client.message('^7Please put the plugin module in ^3@b311/extplugins/')
        except AttributeError:
            client.message('^7Plugin ^1%s ^7has an invalid structure: can\'t load' % name)
            client.message('^7Please inspect your b311 log file for more information')
            ...
            client.message(
                '^7Please put the plugin configuration file in ^3@b311/conf ^7or ^3@b311/extplugins/%s/conf' % name)
            ...
            except b311.exceptions.MissingRequirement:
            ...
            if not plugin:
                ...
                if plugin.isEnabled():
                    ...
                    unreg = [x for x in self.console.cron._tabs
                             if isinstance(self.console.cron._tabs[x], b311.cron.PluginCronTab)
