...
import b311.events
import b311.plugin
from b311.config import NoOptionError
from b311.functions import getCmd


class PoweradminhfPlugin(b311.plugin.Plugin):
    _adminPlugin = None
    ...
    def LoadMatchMode(self):
        ...
        """
        try:
            self._match_plugin_disable = [x.strip() for x in
                                          self.config.get('settings', 'pamatch_plugins_disable').split(',')]
    ...
    def onClientTeamChange(self, event):
        ...
        if self._enableTeamBalancer:
            ...
            if client.team in (b311.TEAM_SPEC, b311.TEAM_UNKNOWN):
                return
    ...
    def onClientVoteStart(self, event):
        ...
        if event.data.lower() in ('kick', 'ban') and event.client \
            ...
            self._adminPlugin.penalizeClient('warning', event.client, duration="2d",
                                             reason="do not call vote against admin")
        ...
        if self._currentVote.data.lower() == 'ban' and event.data['voteresult'].lower() == "passed":
            ...
            if votecaller and votetarget:
                if votetarget.maxLevel < self._auto_unban_level:
                    self.info("%s (%s) is in a lower group than %s, vote allowed", votetarget, votetarget.maxLevel,
                              self._auto_unban_level)
                elif votecaller.maxLevel > votetarget.maxLevel:
                    self.info("%s (%s) is in a higher group than %s (%s), vote allowed", votecaller,
                              votecaller.maxLevel, votetarget, votetarget.maxLevel)
                else:
                    self.info("%s (%s) cannot vote ban %s (%s), vote cancelled", votecaller, votecaller.maxLevel,
                              votetarget, votetarget.maxLevel)
    ...
    def getTeams(self):
        ...
        for c in clients:
            if c.team == b311.TEAM_RED:
                team1players.append(c.cid)
            elif c.team == b311.TEAM_BLUE:
    ...
    def teambalance(self):
        ...
        if gap <= 1:
            self.verbose('teambalance: teams are balanced, T1: %s, T2: %s (diff: %s)',
                         (len(team1players), len(team2players), gap))
            return

        howManyMustSwitch = int(gap / 2)
        bigTeam = b311.TEAM_RED
        if len(team2players) > len(team1players):
            bigTeam = b311.TEAM_BLUE

        self.verbose('teambalance: teams are NOT balanced, T1: %s, T2: %s (diff: %s)',
                     (len(team1players), len(team2players), gap))
    ...
    def cmd_pateams(self, data, client, cmd=None):
        ...
        if client:
            ...
            if gap <= 1:
                client.message(
                    'teams are balanced, T1: %s, T2: %s (diff: %s)' % (len(team1players), len(team2players), gap))
            else:
                self.teambalance()
