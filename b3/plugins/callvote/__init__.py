...
class CallvotePlugin(b311.plugin.Plugin):
    adminPlugin = None
    ...
    def onLoadConfig(self):
        ...
        if self.config.has_section('callvoteminlevel'):
            for o in self.config.options('callvoteminlevel'):
                self.callvoteminlevel[o] = self.getSetting('callvoteminlevel', o, b311.LEVEL, self.callvoteminlevel[o])

        if self.config.has_section('callvotespecialmaplist'):
            for o in self.config.options('callvotespecialmaplist'):
                self.callvotespecialmaplist[o.lower()] = self.getSetting('callvotespecialmaplist', o.lower(),
                                                                         b311.LEVEL)
    ...
    def onStartup(self):
        ...
        if 'callvote' not in self.console.storage.getTables():
            sql_path_main = b311.getAbsolutePath('@b311/plugins/callvote/sql')
