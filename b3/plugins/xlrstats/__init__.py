...
from b311.functions import escape
...
class XlrstatsPlugin(b311.plugin.Plugin):
    ...
    def __init__(self, console, config=None):
        ...
        b311.plugin.Plugin.__init__(self, console, config)
    ...
    def onStartup(self):
        ...
        self.correctStats()
        self._cronTabCorrectStats = b311.cron.PluginCronTab(self, self.correctStats, 0, '0', '*/2')
        ...
        self._cronTabKillBonus = b311.cron.PluginCronTab(self, self.calculateKillBonus, 0, '*/10')
    ...
    def onLoadConfig(self):
        ...
        self.provisional_ranking = self.getSetting('settings', 'provisional_ranking', b311.BOOL,
                                                   self.provisional_ranking)
        self.auto_correct = self.getSetting('settings', 'auto_correct', b311.BOOL, self.auto_correct)
        self.auto_purge = self.getSetting('settings', 'auto_purge', b311.BOOL, self.auto_purge)
        self.silent = self.getSetting('settings', 'silent', b311.BOOL, self.silent)
        self.hide_bots = self.getSetting('settings', 'hide_bots', b311.BOOL, self.hide_bots)
        self.exclude_bots = self.getSetting('settings', 'exclude_bots', b311.BOOL, self.exclude_bots)
        self.min_players = self.getSetting('settings', 'minplayers', b311.INT, self.min_players,
                                           lambda x: int(max(x, 0)))
        self.webfront_version = self.getSetting('settings', 'webfrontversion', b311.STR, self.webfront_version)
        self.webfront_url = self.getSetting('settings', 'webfronturl', b311.STR, self.webfront_url)
        self.webfront_config_nr = self.getSetting('settings', 'servernumber', b311.INT, self.webfront_config_nr,
                                                  validate_server_nr)
        self.keep_history = self.getSetting('settings', 'keep_history', b311.BOOL, self.keep_history)
        self.onemaponly = self.getSetting('settings', 'onemaponly', b311.BOOL, self.onemaponly)
        self.minlevel = self.getSetting('settings', 'minlevel', b311.LEVEL, self.minlevel, lambda x: int(max(x, 0)))
        self.defaultskill = self.getSetting('settings', 'defaultskill', b311.INT, self.defaultskill)
        self.Kfactor_high = self.getSetting('settings', 'Kfactor_high', b311.INT, self.Kfactor_high)
        self.Kfactor_low = self.getSetting('settings', 'Kfactor_low', b311.INT, self.Kfactor_low)
        self.Kswitch_confrontations = self.getSetting('settings', 'Kswitch_confrontations', b311.INT,
                                                      self.Kswitch_confrontations)
        self.steepness = self.getSetting('settings', 'steepness', b311.INT, self.steepness)
        self.suicide_penalty_percent = self.getSetting('settings', 'suicide_penalty_percent', b311.FLOAT,
                                                       self.suicide_penalty_percent)
        self.tk_penalty_percent = self.getSetting('settings', 'tk_penalty_percent', b311.FLOAT, self.tk_penalty_percent)
        self.assist_timespan = self.getSetting('settings', 'assist_timespan', b311.INT, self.assist_timespan)
        self.damage_assist_release = self.getSetting('settings', 'damage_assist_release', b311.INT,
                                                     self.damage_assist_release)
        self.prematch_maxtime = self.getSetting('settings', 'prematch_maxtime', b311.INT, self.prematch_maxtime)
        self.announce = self.getSetting('settings', 'announce', b311.BOOL, self.announce)
        self.keep_time = self.getSetting('settings', 'keep_time', b311.BOOL, self.keep_time)
    ...
    def build_database_schema(self):
        ...
        """
        sql_main = os.path.join(b311.getAbsolutePath('@b311/plugins/xlrstats/sql'), self.console.storage.protocol)
    ...
    def dataReset(self, pid):
        q = """UPDATE xlr_playerstats SET kills=0, deaths=0, teamkills=0, teamdeaths=0, suicides=0, ratio=0, skill=0, assists=0, assistskill=0, curstreak=0, winstreak=0, losestreak=0, rounds=0, hide=0, fixed_name='', id_token='' WHERE id= %s""" % (
            pid)
        self.query(q)
...
# class XlrstatscontrollerPlugin(b311.plugin.Plugin):
...
#         # empty message cache
#         self._messages = {}
#         self.registerEvent(b311.events.EVT_STOP)
#         self.registerEvent(b311.events.EVT_EXIT)
#
#     def onStartup(self):
...
#         # register the events we're interested in.
#         self.registerEvent(b311.events.EVT_CLIENT_JOIN)
#         self.registerEvent(b311.events.EVT_GAME_ROUND_START)
#
#     def onEvent(self, event):
#         if event.type == b311.events.EVT_CLIENT_JOIN:
#             self.checkMinPlayers()
#         elif event.type == b311.events.EVT_GAME_ROUND_START:
#             self.checkMinPlayers(_roundstart=True)
#
#     def checkMinPlayers(self, _roundstart=False):
#         """Checks if minimum amount of players are present
...
class XlrstatshistoryPlugin(b311.plugin.Plugin):
    """
    ...
    def __init__(self, console, weeklyTable, monthlyTable, playerstatsTable):
        ...
        """
        b311.plugin.Plugin.__init__(self, console)
        ...
        self.query = self.console.storage.query
        # purge crontab
        tzName = self.console.config.get('b311', 'time_zone').upper()
        tzOffest = b311.timezones.timezones[tzName]
        ...
        self._cronTab = b311.cron.PluginCronTab(self, self.purge, 0, self._minutes, hoursGMT, '*', '*', '*')
    ...
    def onStartup(self):
        ...
        try:
            # install crontabs
            self._cronTabMonth = b311.cron.PluginCronTab(self, self.snapshot_month, 0, 0, 0, 1, '*', '*')
            self.console.cron + self._cronTabMonth
            self._cronTabWeek = b311.cron.PluginCronTab(self, self.snapshot_week, 0, 0, 0, '*', '*',
                                                        1)  # day 1 is monday
            self.console.cron + self._cronTabWeek
        except Exception as msg:
...
class CtimePlugin(b311.plugin.Plugin):
    """
...
class BattlestatsPlugin(b311.plugin.Plugin):
    """
