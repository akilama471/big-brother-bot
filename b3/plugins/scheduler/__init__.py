...
class SchedulerPlugin(b311.plugin.Plugin):
    _tasks = None
    _tzOffset = 0
    _restart_tasks = set()
    ...
    def onLoadConfig(self):
        ...
        self._tzOffset = b311.timezones.timezones[tzName]
...
class RestartTask(Task):
    ...
    def runcommands(self):
        if 'delay' in self.config.attrib:
            delay_minutes = b311.functions.time2minutes(self.config.attrib['delay'])
...
class CronTask(Task):
    ...
    def schedule(self):
        ...
        self.cronTab = b311.cron.PluginCronTab(self.plugin, self.runcommands,
