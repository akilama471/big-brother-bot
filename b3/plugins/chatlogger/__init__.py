...
import b311
from b311.config import ConfigParser
from b311.cron import PluginCronTab
from b311.plugin import Plugin
from b311.timezones import timezones
...
class ChatloggerPlugin(Plugin):
    ...
    def onStartup(self):
        ...
        if 'chatlog' not in current_tables or 'cmdlog' not in current_tables:
            sql_path_main = b311.getAbsolutePath('@b311/plugins/chatlogger/sql')
    ...
    def purge(self):
        ...
        if self._max_age_cmd_in_days and (self._max_age_cmd_in_days != 0):
            ...
            q = "DELETE FROM %s WHERE cmd_time < %i" % (self._db_table_cmdlog,
                                                        self.console.time() - (
                                                                    self._max_age_cmd_in_days * 24 * 60 * 60))
            self.debug(q)
            self.console.storage.query(q)
        else:
...
class CmdData(AbstractData):
    ...
    def save(self):
        self.plugin.verbose(
            "%s, %s, %s, %s, %s" % (self.admin_id, self.admin_name, self.command, self.data, self.result))
