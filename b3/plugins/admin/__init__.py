...
class AdminPlugin(b311.plugin.Plugin):
    _commands = {}
    ...
    def onStartup(self):
        ...
        if not self.console.storage.db:
            ...
            try:
                ...
                self.critical('seems your groups table in the database is empty: please recreate your database using '
                              ...
                              'script: %s - (%s)' % (
                              b311.getAbsolutePath("@b311/sql/%s/b311.sql" % self.console.storage.dsnDict['protocol']),
                              msg))

            if 'iamakr' in self._commands and \
                    self._commands['iamakr'].level is not None and \
                    self._commands['iamakr'].level[0] >= 0:
                ## here the config file for the admin plugin explicitly enables the iamakr command
                if len(superadmins) == 0:
                    self.verbose('!iamakr command enabled by config file: be sure to disable it after typing !iamakr.')
                else:
                    self.warning('!iamakr command enabled by config file but %s superadmin are already registered: ' +
                                 'make sure to disable the iamakr command in the admin plugin', len(superadmins))
            elif len(superadmins) == 0:
                self.verbose('no superadmin found: enabling !iamakr')
                # There are no superadmins, enable the !iamakr command
                self.registerCommand(self, 'iamakr', 0, getCmd(self, 'iamakr'))
            else:
                self.verbose('superadmin(s) found: no need for !iamakr')
        ...
        if self._past_bans_check_rate > 0:
            ...
            self._past_bans_crontab = b311.cron.PluginCronTab(self, self.doPastBansCheck, minute='*',
    ...
    def cmd_b3(self, data, client, cmd=None):
        """
        - say b311's version info
        """
        if len(data) > 0 and client.maxLevel >= self._admins_level:
            data = data.lower().strip()
            if data == 'poke':
                self.console.say('^7Do not poke b311 %s^7!' % client.exactName)
            elif data == 'expose':
                self.console.say('^7Do expose b311 to sunlight %s^7!' % client.exactName)
            elif data == 'stare':
                self.console.say('^7Do not stare at b311 %s^7!' % client.exactName)
            elif data == 'stab':
                self.console.say('^7b3 is invincible, %s^7 could not penetrate he hide of b311' % client.exactName)
            elif data == 'bite':
                ...
                self.console.say('^7%s^7 is knocked out by b311' % client.exactName)
            elif data == 'feed':
                ...
                self.console.say('^7Your hit detection is off %s^7, b311 is unharmed' % client.exactName)
            elif data == 'kick':
            ...
            cmd.sayLoudOrPM(client, '%s ^7- uptime: [^2%s^7]' % (
                b311.version, functions.minutesStr(self.console.upTime() / 60.0)))
    ...
    def cmd_ad(self, data, client, cmd=None):
        """
        - lists all the online admins
        """
        self.debug('trying to get admins')
    ...
    def cmd_pg(self, data, client, cmd=None):
        """
        <client> <group> - add a client to a group
        """
        m = re.match('^(.{2,}) ([a-z][a-z0-9]+)$', data, re.I)
    ...
    def cmd_ug(self, cid, client, cmd=None):
        """
        <client> <group> - remove a client from a group
        """
        m = re.match('^([^ ]{2,}) ([a-z]+)$', cid)
    ...
    def cmd_iamakr(self, data, client, cmd=None):
        """
        ...
        if len(superadmins) > 0:
            ...
            if 'iamakr' in self._commands:
                self._commands.pop('iamakr')
            return
        ...
        try:
            ...
            try:
                command = self._commands['iamakr']
            except KeyError:
                self.error('iamakr command not found')
                return False
            else:
                command.level = 'none'
                if group in client.groups:
                    client.message('^7Mother Fucker, You are already a ^2%s' % group.exactName)
                else:
                    client.setGroup(group)
                    client.save()
                    client.message('^7Mother Fucker, You are now a ^2%s' % group.name)
    ...
    def cmd_sc(self, data, client=None, cmd=None):
        """
        <message> - yell a message to all player
        """
        thread.start_new_thread(self.sayMany, (data, 5, 1))
    ...
    def cmd_poke(self, data, client=None, cmd=None):
        ...
        if not m:
            ...
            if m[0] == 'b311':
                self.warnClient(client, 'Do not poke b311!', None, False, '', 1)
            else:
                sclient = self.findClientPrompt(m[0], client)
                if sclient:
