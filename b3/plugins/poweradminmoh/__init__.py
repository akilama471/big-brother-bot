...
class PoweradminmohPlugin(b311.plugin.Plugin):
    requiresParsers = ['moh']
    ...
    def loadTeamBalancer(self):
        ...
        if self._tinterval > 0:
            self._tcronTab = b311.cron.PluginCronTab(self, self.autobalance, 0, '*/%s' % self._tinterval)
            self.console.cron + self._tcronTab
    ...
    def teambalance(self):
        ...
        playersToMove = [c for (c, teamtime) in sortedPlayersTeamTimes if c.maxLevel < self._tmaxlevel][
                        :howManyMustSwitch]
