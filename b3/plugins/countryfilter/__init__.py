...
class CountryfilterPlugin(b311.plugin.Plugin):
    ...
    def onLoadConfig(self):
        ...
        self.cf_announce_accept = self.getSetting('settings', 'cf_announce_accept', b311.BOOL, self.cf_announce_accept)
        self.cf_announce_reject = self.getSetting('settings', 'cf_announce_reject', b311.BOOL, self.cf_announce_reject)
        self.cf_message_exclude_from = self.getSetting('settings', 'cf_message_exclude_from', b311.STR,
                                                       self.cf_message_exclude_from)
        self.cf_order = self.getSetting('settings', 'cf_order', b311.STR, self.cf_order)
        self.cf_deny_from = self.getSetting('settings', 'cf_deny_from', b311.STR, self.cf_deny_from)
        self.cf_allow_from = self.getSetting('settings', 'cf_allow_from', b311.STR, self.cf_allow_from)
        self.maxLevel = self.getSetting('settings', 'maxlevel', b311.LEVEL, self.maxLevel)
