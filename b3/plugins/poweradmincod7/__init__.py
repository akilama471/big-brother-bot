...
class Poweradmincod7Plugin(b311.plugin.Plugin):
    ...
    def cmd_palistcfg(self, data, client, cmd):
        """
        List available server config files in b311 conf folder
        (You can safely use the command without the 'pa' at the beginning)
        """
        config_files = []
        filenames = os.listdir(b311.getConfPath())
    ...
    def cmd_paload(self, data, client, cmd=None):
        ...
        if not data.endswith('.cfg'):
            ...
            if not os.path.isfile(os.path.join(b311.getConfPath(), data)):
    ...
    def _configloader(self, configfile):
        ...
        """
        with open(os.path.join(b311.getConfPath(), configfile), 'r') as f:
            for line in f:
