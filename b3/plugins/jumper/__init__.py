...
class JumperPlugin(b311.plugin.Plugin):
    adminPlugin = None
    ...
    def onLoadConfig(self):
        ...
        """
        self._demo_record = self.getSetting('settings', 'demorecord', b311.BOOL, self._demo_record)
        self._skip_standard_maps = self.getSetting('settings', 'skipstandardmaps', b311.BOOL, self._skip_standard_maps)
        self._min_level_delete = self.getSetting('settings', 'minleveldelete', b311.LEVEL, self._min_level_delete)
    ...
    def onStartup(self):
        ...
        if 'jumpruns' not in current_tables or 'jumpways' not in current_tables:
            sql_path_main = b311.getAbsolutePath('@b311/plugins/jumper/sql')
