...
class FtpytailPlugin(b311.plugin.Plugin):
    ...
    def onLoadConfig(self):
        ...
        """
        # These values go in b311 xml, so check them first
        try:
        ...
        try:
            ...
            self.debug(
                'using default value (%s) for settings/max_consecutive_failures' % self._maxConsecutiveConnFailure)
...
if __name__ == '__main__':
    ...
    print
    "------------------------------------"
    config = b311.config.XmlConfigParser()
    config.setXml("""
