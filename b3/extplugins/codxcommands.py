...
class CodxcommandsPlugin(b311.plugin.Plugin):
    _adminPlugin = None
    ...
    def cmd_s(self, data, client, cmd=None):
        """
        <text> - say <text> to the server.
        """
        if not data:
    ...
    def cmd_ss(self, data, client, cmd=None):
        ...
        """
        ss_site_link = self.config.get('akarisquad', 'https://www.akarisquad.com/ss/')
        if not data:
            client.message('^7Missing data, usage: getss <player name>')
        elif data.lower() == 'all':
            client.message('Taking screenshots of all players. Visit ^1' + ss_site_link + ' ^7to view screenshots.')
            ...
            if not target:
                ...
                client.message(
                    'Screenshot of %s will be taken soon. Visit ^1' + ss_site_link + ' ^7to view screenshots.' % target.exactName)

    def cmd_ssall(self, data, client, cmd=None):
        """
        <client> - name or partial name of the player. Alternatively you can use 'all' to take
        screenshot of all players.
        """
        ss_site_link = self.config.get('akarisquad', 'screenshot_website_link')
        if not data:
            client.message('Taking screenshots of all players. Visit ^1' + ss_site_link + ' ^7to view screenshots.')
            self.console.write('getss all')
        else:
            client.message('^7Type only !ssall to Taking screenshots of all players. ')
