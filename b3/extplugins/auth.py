...
class AuthPlugin(b311.plugin.Plugin):
    ...
    def onLoadConfig(self):
        ...
        self._need_auth = self._get_setting('messages', 'auth_message',
                                            default='^7Please authenticate yourself with ^3!auth <password>')
        self._auth_done = self._get_setting('messages', 'auth_done', default='^3You have been authenticated!')
        self._already_done = self._get_setting('messages', 'already_done',
                                               default='^3You have already been authenticated!')
        self._bad_auth = self._get_setting('messages', 'bad_auth', default='^1Wrong password!')
        self._no_password = self._get_setting('messages', 'no_password',
                                              default='^1Missing password!^7 Usage: !auth <password>')
    ...
    def onEvent(self, event):
        if (event.type == b311.events.EVT_CLIENT_AUTH):
