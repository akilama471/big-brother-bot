...
import b311.game
import b311.output
import b311.parsers.q3a.rcon
import b311.storage
import b311.timezones
from b311 import __version__ as currentVersion
from b311.clients import Clients
from b311.clients import Group
from b311.decorators import Memoize
from b311.exceptions import MissingRequirement
from b311.functions import getModule
from b311.functions import main_is_frozen
from b311.functions import right_cut
from b311.functions import splitDSN
from b311.functions import topological_sort
from b311.functions import vars2printf
from b311.plugin import PluginData
from b311.update import B3version
...
class Parser(object):
    OutputClass = b311.parsers.q3a.rcon.Rcon  # default output class set to the q3a rcon class
    ...
    name = 'b311'  # bot name
    output = None  # will contain the instance used to send data to the game server (default to b311.parsers.q3a.rcon.Rcon)
    ...
    def __init__(self, conf, options):
        ...
        # set up logging
        logfile = self.config.getpath('b311', 'logfile')
        ...
        logfile = b311.getWritableFilePath(logfile, True)

        try:
            logsize = b311.functions.getBytes(self.config.get('b311', 'logsize'))
        except (TypeError, NoOptionError):
            logsize = b311.functions.getBytes('10MB')

        # create the main logger instance
        self.log = b311.output.getInstance(logfile, self.config.getint('b311', 'log_level'), logsize, log2console)

        # save screen output to self.screen
        self.screen = sys.stdout
        self.screen.write(
            'Activating log   : %s\n' % b311.getShortPath(os.path.abspath(b311.getAbsolutePath(logfile, True))))
        self.screen.flush()

        sys.stdout = b311.output.STDOutLogger(self.log)
        sys.stderr = b311.output.STDErrLogger(self.log)
        ...
        if self._publicIp and self._publicIp[0:1] in ('~', '/'):
            # load ip from a file
            f = file(b311.getAbsolutePath(self._publicIp, decode=True))
            self._publicIp = f.read().strip()
            f.close()

        if self._rconIp[0:1] in ('~', '/'):
            # load ip from a file
            f = file(b311.getAbsolutePath(self._rconIp, decode=True))
        ...
        self.bot('%s', b311.getB3versionString())
        ...
        # get events
        self.Events = b311.events.eventManager
        self._eventsStats = b311.events.EventsStats(self)

        self.bot('--------------------------------------------')

        # setup bot
        bot_name = self.config.get('b311', 'bot_name')
        if bot_name:
            self.name = bot_name

        bot_prefix = self.config.get('b311', 'bot_prefix')
        if bot_prefix:
        ...
        try:
            ...
            self.storage = b311.storage.getStorage(dsn=dsn, dsnDict=splitDSN(dsn), console=self)
        ...
        self.game = b311.game.Game(self, self.gameName)

        try:
            queuesize = self.config.getint('b311', 'event_queue_size')
    ...
    def getAbsolutePath(self, path, decode=False):
        ...
        """
        return b311.getAbsolutePath(path, decode=decode)
    ...
    def start(self):
        ...
        self.startPlugins()
        self._eventsStats_cronTab = b311.cron.CronTab(self._dumpEventsStats)
    ...
    def getEvent(self, key, data=None, client=None, target=None):
        ...
        """
        return b311.events.Event(self.Events.getId(key), data, client, target)
    ...
    def loadPlugins(self):
        ...
        def _get_plugin_config(p_name, p_clazz, p_config_path=None):
            ...
            def _search_config_file(match):
                ...
                search = '%s%s*%s*' % (b311.getAbsolutePath('@conf\\', decode=True), os.path.sep, match)
                ...
                search = '%s%s*%s*' % (
                os.path.join(b311.getAbsolutePath(extplugins_dir, decode=True), match, 'conf'), os.path.sep, match)
                self.debug('Searching for configuration file(s) matching: %s' % search)
                collection = glob.glob(search)
                return collection
            ...
            if p_config_path is None:
                ...
                if len(search_path) == 0:
                    ...
                    raise b311.config.ConfigFileNotFound('could not find any configuration file for plugin %s' % p_name)
                ...
                return b311.config.load(search_path[0])
            else:
                ...
                if p_clazz.requiresConfigFile:
                    ...
                    raise b311.config.ConfigFileNotFound(
        ...
        plugin_required = []  # hold a list of required plugin names which have not been specified in b311.ini
        ...
        # here below we will parse the plugins section of b311.ini, looking for plugins to be loaded.
        ...
        # at this point we have an OrderedDict of PluginData of plugins listed in b311.ini and which can be loaded correctly:
        # all the plugins which have not been installed correctly, but are specified in b311.ini, have been already excluded.
        ...
        # make sure that required plugins are enabled (both if loaded in b311.ini or loaded automatically)
    ...
    def pluginImport(self, name, path=None):
        ...
        try:
            fp, pathname, description = imp.find_module(name, [os.path.join(b311.getB3Path(True), 'plugins')])
            return imp.load_module(name, fp, pathname, description)
    ...
    def getTzOffsetFromName(self, tz_name=None):
        ...
        """
        if tz_name:
            if not tz_name in b311.timezones.timezones:
                self.warning(
                ...
                self.info("Using timezone: %s : %s" % (tz_name, b311.timezones.timezones[tz_name]))
                return b311.timezones.timezones[tz_name], tz_name
    ...
    def formatTime(self, gmttime, tz_name=None):
        """
        Return a time string formatted to local time in the b311 config time_format
        ...
        if tz_name:
            ...
            # use the timezone name specified in b311 main configuration file (if specified),
            # or make use of the timezone offset autodetection implemented in getTzOffsetFromName
            tz_name = None
            if self.config.has_option('b311', 'time_zone'):
                tz_name = self.config.get('b311', 'time_zone').strip().upper()
                tz_name = tz_name if tz_name and tz_name != 'AUTO' else None
            tz_offset, tz_name = self.getTzOffsetFromName(tz_name)

        time_format = self.config.get('b311', 'time_format').replace('%Z', tz_name).replace('%z', tz_name)
    ...
    def parseLine(self, line):
        ...
        if m:
            self.queueEvent(b311.events.Event(self.getEventID('EVT_UNKNOWN'), m.group(2)[:1]))
        ...
        try:
            if self.working and self.exiting.acquire():
                ...
                for k, plugin in self._plugins.items():
                    plugin.parseEvent(b311.events.Event(self.getEventID('EVT_STOP'), ''))
                if self._cron:
    ...
    def finalize(self):
        ...
        Called internally by b311.parser.die()
        """
        if b311.getPlatform() in ('linux', 'darwin'):
            ...
            pidpath = os.path.join(b311.getAbsolutePath('@b311/', decode=True), '..', 'scripts', 'pid',
                                   '%s.pid' % b3_name)
    ...
    def _get_cron(self):
        ...
        """
        if not self._cron:
            self._cron = b311.cron.Cron(self)
            self._cron.start()
        return self._cron
    ...
    def updateDocumentation(self):
        ...
        if self.config.has_section('autodoc'):
            try:
                from b311.tools.documentationBuilder import DocBuilder
