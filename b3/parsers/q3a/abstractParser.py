...
from b311.parsers.punkbuster import PunkBuster
from b311.parsers.q3a import rcon
...
class AbstractParser(b311.parser.Parser):
    ...
    def smartSay(self, client, text):
        ...
        :param text: The message to be sent.
        """
        if client and (client.state == b311.STATE_DEAD or client.team == b311.TEAM_SPEC):
    ...
    def sayDead(self, text):
        ...
        for client in self.clients.getClientsByState(b311.STATE_DEAD):
            if client.cid:
                for line in wrapped:
                    lines.append(self.getCommand('message', cid=client.cid, message=line))
        self.writelines(lines)
            ...
            if m:
                if m.group('cvar').lower() == cvar_name.lower():
                    try:
                        ...
                        default_value = None
                    return b311.cvar.Cvar(m.group('cvar'), value=m.group('value'), default=default_value)
            else:
                return None
