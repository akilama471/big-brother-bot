...
import b311.clients
import b311.events
from b311.parsers.frostbite2.abstractParser import AbstractParser
...
BF4_SPECTATOR = 1  # spectator which is not visible in the game for other player but visible as player for b311
BF4_COMMANDER = 2  # commander which is visible for other player and b311
...
class Bf4Parser(AbstractParser):
    ...
    def startup(self):
        ...
        self.clients.newClient('Server', guid='Server', name='Server', hide=True,
                               pbid='Server', team=b311.TEAM_UNKNOWN, squad=None)
        ...
        if not client:
            if cid == 'Server':
                return self.clients.newClient('Server', guid='Server', name='Server', hide=True,
                                              pbid='Server', team=b311.TEAM_UNKNOWN, teamId=None, squadId=None)
            if guid:
                client = self.clients.newClient(cid, guid=guid, name=cid, team=b311.TEAM_UNKNOWN, teamId=None,
                                                squad=None)
            else:
    ...
    def getTeam(self, team, type_id=None):
        ...
        if type_id and int(type_id) == BF4_SPECTATOR:
            return b311.TEAM_SPEC
        elif team == 1:
            return b311.TEAM_RED
        elif team == 2:
            return b311.TEAM_BLUE
        elif team == 0:
            return b311.TEAM_SPEC
        else:
            return b311.TEAM_UNKNOWN
    ...
    def patch_b3_Client_properties():
        ...
        def getPlayerState(self):
            _state = b311.STATE_UNKNOWN
            _isAlive = self.isAlive()
            if _isAlive:
                _state = b311.STATE_ALIVE
            elif _isAlive is False:
                _state = b311.STATE_DEAD
            return _state
        ...
        b311.clients.Client.isAlive = isAlive
        b311.clients.Client.state = property(getPlayerState, setPlayerState)
        b311.clients.Client._get_player_type = _get_player_type
        b311.clients.Client.player_type = property(get_player_type, set_player_type)
        b311.clients.Client.is_commander = property(get_commander_state, set_commander_state)
