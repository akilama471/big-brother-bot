...
from b311.parsers.frostbite2.util import MapListBlockError
    ...
    def pluginsStarted(self):
        ...
        for cid, p in plist.iteritems():
            ...
            if not client:
                ...
                client = self.clients.newClient(cid, guid=p['name'], name=p['name'], team=p['teamId'],
        ...
        if not client:
            if cid == 'Server':
                return self.clients.newClient('Server', guid='Server', name='Server',
                                              hide=True, pbid='Server', team=b311.TEAM_UNKNOWN,
                                              teamId=None, squadId=None)
            if guid:
                client = self.clients.newClient(cid, guid=guid, name=cid, team=b311.TEAM_UNKNOWN,
...
class NewMapListBlock(b311.parsers.frostbite2.util.MapListBlock):
    """
    ...
    def append(self, data):
        ...
        if type(data) not in (tuple, list):
            raise MapListBlockError(
                "invalid data: expecting data as a tuple or as a list, received '%s' instead" % type(data))
        ...
        for i in range(num_maps):
            ...
            map_data.append(
                {'name': data[base_index + 0], 'gamemode': data[base_index + 1], 'num_of_rounds': num_rounds})
