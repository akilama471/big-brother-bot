...
class AltitudeParser(Parser):
    ...
    """
    ## parser code name to use in b311.ini
    gameName = 'altitude'
    ...
    def _initialize_rcon(self):
        ...
        """
        # check that the command file is provided in the b311.xml config file
    ...
    def parseLine(self, line):
        ...
        if not hasattr(self, method_name):
            if event_type in self._eventMap:
                event = b311.events.Event(self._eventMap[event_type], data=altitude_event)
            else:
    ...
    def OnAltitudeClientAdd(self, altitude_event):
        ...
        client = self.newClient(altitude_event['player'], guid=vaporId,
                                name=altitude_event['nickname'], team=b311.TEAM_UNKNOWN,
    ...
    def OnAltitudeStructureDamage(self, altitude_event):
        ...
        if 'xp' in altitude_event:
            attacker.currentXP = int(altitude_event['xp'])
        return self.getEvent("EVT_CLIENT_ACTION", data="structureDamage", client=attacker,
                             target=altitude_event['target'])
    ...
    def OnAltitudeStructureDestroy(self, altitude_event):
        ...
        if client:
            ...
            return self.getEvent("EVT_CLIENT_ACTION", data="structureDestroy", client=client,
                                 target=altitude_event['target'])
    ...
    def kick(self, client, reason='', admin=None, silent=False, *kwargs):
        ...
        """
        if not isinstance(client, b311.clients.Client):
