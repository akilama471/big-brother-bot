...
#                                 - set client.state to b311.STATE_ALIVE on Client Spawn
...
# 25/01/2015 - 1.29   - Fenix     - patch the b311.clients.getByMagic method so it's possible to lookup players using their
#                                   auth login
...
from b311.events import Event
from b311.functions import time2minutes
from b311.parsers.iourt41 import Iourt41Parser
from b311.plugins.spamcontrol import SpamcontrolPlugin
...
class Iourt42Client(Client):
    ...
    def auth(self):
        """
        The b311.clients.Client.auth method needs to be changed to fit the UrT4.2 authentication scheme.
        In UrT4.2 :
...
class Iourt42Parser(Iourt41Parser):
    ...
    def load_conf_permban_with_frozensand(self):
        """
        Load permban configuration from b311.xml.
        """
    ...
    def load_conf_tempban_with_frozensand(self):
        """
        Load tempban configuration from b311.xml.
        """
            ...
            if client:
                ...
                for k, v in bclient.iteritems():
                    if hasattr(client, 'gear') and k == 'gear' and client.gear != v:
                        self.queueEvent(b311.events.Event(self.getEventID('EVT_CLIENT_GEAR_CHANGE'), v, client))
    ...
    def patch_Clients():
        ...
        b311.clients.Clients.newClient = newClient
        b311.clients.Clients.getByMagic = newGetByMagic
