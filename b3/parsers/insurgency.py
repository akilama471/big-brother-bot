...
from b311.parsers.source.rcon import Rcon
...
# disable the authorizing timer that comes by default with the b311.clients.Clients class
...
class InsurgencyParser(Parser):
    ...
    def on_kill(self, a_name, a_cid, a_guid, a_team, v_name, v_cid, v_guid, v_team, weapon, properties):
        ...
        # victim.state = b311.STATE_DEAD ## do we need that ? is this info used ?
    ...
    @ger.gameEvent(
        r'^"(?P<name>.+)<(?P<cid>\d+)><(?P<guid>.+)><(?P<team>.*)>"(?: \[-?\d+ -?\d+ -?\d+\])? committed suicide with "(?P<weapon>\S*)"$')
    ...
    @ger.gameEvent(
        r'^"(?P<name>.+)<(?P<cid>\d+)><(?P<guid>.+)><(?P<team>.*)>" triggered "(?P<event_name>\S+)"(?P<properties>.*)$')
    ...
    @ger.gameEvent(
        r'^"(?P<name>.+)<(?P<cid>\d+)><(?P<guid>.+)><(?P<team>\S+)>" threw (?P<item>.+?)( \[-?\d+ -?\d+ -?\d+\])?$')
    ...
    def on_bad_rcon_password(self, ip, port):
        ...
        self.error("Bad RCON password, check your b311.xml file")

    @ger.gameEvent(
        r'^Molotov projectile spawned at (?P<coord>-?[\d.]+ -?[\d.]+ -?[\d.]+), velocity (?P<velocity>-?[\d.]+ -?[\d.]+ -?[\d.]+)$')
    ...
    @ger.gameEvent(
        r'^Banid: "(?P<name>.+)<(?P<cid>\d+)><(?P<guid>.+)><(?P<team>.*)>" was banned "for (?P<duration>.+)" by "(?P<admin>.*)"$')
    ...
    @ger.gameEvent(
        r'^\[basecommands.smx\] ".+<\d+><.+><.*>" kicked "(?P<name>.+)<(?P<cid>\d+)><(?P<guid>.+)><(?P<team>.*)>"(?P<properties>.*)$')
