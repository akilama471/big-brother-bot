...
from b311.output import VERBOSE2
from b311.parsers.battleye.protocol import BattleyeError
from b311.parsers.battleye.protocol import BattleyeServer
from b311.parsers.battleye.protocol import CommandError
from b311.parsers.battleye.protocol import CommandFailedError
from b311.parsers.battleye.rcon import Rcon as BattleyeRcon
...
class AbstractParser(b311.parser.Parser):
    ...
    def startup(self):
        ...
        self.cron.add(b311.cron.CronTab(self.clients.sync, minute='*/1'))
    ...
    def run(self):
        ...
        try:
            b3_log_level = self.config.getint('b311', 'log_level')
        except:
    ...
    def load_protocol_logging(self):
        if self.config.has_option('b311', 'protocol_log'):
            logfile = self.config.getpath('b311', 'protocol_log')
            return logfile
        else:
            return None

    def load_use_unverified_guid(self):
        if self.config.has_option('b311', 'use_unverified_guid'):
            self._useunverifiedguid = self.config.getboolean('b311', 'use_unverified_guid')
    ...
    def OnServerMessage(self, data):
        ...
        # event_type = b311.events.EVT_CLIENT_SAY
        # evt = b311.events.Event(event_type, None, None)
        # pass
        return
