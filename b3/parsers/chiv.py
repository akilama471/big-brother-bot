...
# 1.4 - add debug messages to expose the RCON communication (when log level is down to 8 in b311.xml)
...
class ChivParser(Parser):
    ...
    def handlePacket(self, packet):
        ...
        if packet.msgType == MessageType.PLAYER_CHAT:
            ...
            client = self.clients.getByGUID(playerId)
            if not client:
                client = self.clients.newClient(name, guid=playerId, name=name, team=b311.TEAM_UNKNOWN)
            ...
            if attacker.team != b311.TEAM_UNKNOWN and attacker.team == victim.team:
        ...
        def cmd_find(self, data, client=None, cmd=None):
            ...
            if sclient:
                cmd.sayLoudOrPM(client,
                                '^7Found player matching %s [^2@%s^7] %s' % (cid, sclient.id, sclient.exactName))
