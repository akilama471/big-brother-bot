...
import b311.events
import b311.parser
import b311.parsers.frostbite.rcon as rcon
from b311.functions import levenshteinDistance
from b311.functions import prefixText
from b311.functions import soundex
from b311.parsers.frostbite.connection import FrostbiteCommandFailedError
from b311.parsers.frostbite.connection import FrostbiteConnection
from b311.parsers.frostbite.connection import FrostbiteException
...
class AbstractParser(b311.parser.Parser):
    ...
    _eventMap = {
        # 'player.onKicked': b311.events.EVT_CLIENT_KICK,
    }
            ...
            if self._paused:
                ...
                try:
                    if self._serverConnection is None:
                        ...
                        self._serverConnection = FrostbiteConnection(self, self._rconIp, self._rconPort,
                                                                     self._rconPassword)
    ...
    def routeFrostbitePacket(self, packet):
        ...
        if match and hasattr(self, func):
            ...
            self.queueEvent(b311.events.Event(self._eventMap[eventType], eventData))
        else:
            if func:
                data = func + ' '
    ...
    def OnPlayerAuthenticated(self, action, data):
        ...
        # return b311.events.Event(b311.events.EVT_CLIENT_CONNECT, data, client)
    ...
    def ban(self, client, reason='', admin=None, silent=False, *kwargs):
        ...
        if isinstance(client, b311.clients.Client):
