...
#                                        with missing PB line in b311.xml
...
from b311.parsers.q3a.abstractParser import AbstractParser
...
class Iourt41Parser(AbstractParser):
    ...
    _eventMap = {
        # 'warmup' : b311.events.EVT_GAME_WARMUP,
        # 'shutdowngame' : b311.events.EVT_GAME_ROUND_END
    }
    ...
    def unpause(self):
        ...
        self.clients.sync()
        b311.parser.Parser.unpause(self)
    ...
    def OnClientconnect(self, action, data, match=None):
        ...
        # return b311.events.Event(b311.events.EVT_CLIENT_JOIN, None, client)
    ...
    def OnClientbegin(self, action, data, match=None):
        ...
        if client:
            return b311.events.Event(self.getEventID('EVT_CLIENT_JOIN'), data=data, client=client)
        ...
        if bclient:
            ...
            if client:
                ...
                self.clients.newClient(bclient['cid'], name=bclient['name'], ip=bclient['ip'],
                                       state=b311.STATE_ALIVE, guid=guid, bot=bot, data={'guid': guid})
    ...
    def inflictCustomPenalty(self, penalty_type, client, reason=None, duration=None, admin=None, data=None):
        ...
        if penalty_type == 'slap' and client:
            ...
            if duration is None:
                ...
                seconds = round(float(b311.functions.time2minutes(duration) * 60), 0)
    ...
    def getTeam(self, team):
        ...
        if str(team).lower() == 'red':
            ...
            team = -1  # will fall back to b311.TEAM_UNKNOWN
    ...
    def getPlayerTeams(self):
        ...
        if g_blueteamlist:
            for letter in g_blueteamlist:
                player_teams[letters2slots[letter]] = b311.TEAM_BLUE

        g_redteamlist = cvars.get('g_redteamlist')
        if g_redteamlist:
            for letter in g_redteamlist:
                player_teams[letters2slots[letter]] = b311.TEAM_RED
        return player_teams
