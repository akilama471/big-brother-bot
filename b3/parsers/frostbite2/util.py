...
class BanlistContent(object):
    ...
    def append(self, data):
        ...
        if len(data) % 6 != 0:
            raise BanlistContentError(
                "invalid data: the total number of elements is not divisible by 6 (%s)" % len(data))
        ...
        for i in range(num_maps):
            ...
            try:
                ...
                raise MapListBlockError(
                    "invalid data: %sth element should be a integer, got %r" % (base_index + 2, data[base_index + 2]))
            map_data.append(
