...
class FrontlineParser(b311.parser.Parser):
    """
    ...
    def getClientOrCreate(self, cid, guid, name):
        ...
        client = self.clients.getByCID(cid)
        if client is None:
            client = self.clients.newClient(cid, guid=guid, name=name, team=b311.TEAM_UNKNOWN)
    ...
    @ger.gameEvent(
        r'^.*: Player GUID Computed (?P<pbid>[0-9a-f]+)\(-\) \(slot #(?P<cid>\d+)\) (?P<ip>[0-9.]+):(?P<port>\d+) (?P<name>.+)$')
    ...
    @ger.gameEvent(
        r'^.*: (?P<cid>\d+)\s+(?P<pbid>[a-z0-9]+)?\(-\) (?P<ip>[0-9.]+):(?P<port>\d+) (\w+)\s+(\d+)\s+([\d.]+)\s+(\d+)\s+\((.)\) "(?P<name>.+)"$')
    ...
    @ger.gameEvent(re.compile(
        r'^Banned Player: PlayerName="(?P<name>.+)" PlayerID=(?P<cid>(-1|\d+)) ProfileID=(?P<guid>\d+) Hash=(?P<hhash>.*) BanDuration=(?P<duration>-?\d+)( Permanently)?$',
        re.MULTILINE))
    ...
    @ger.gameEvent(re.compile(
        r'^UnBanned Player: PlayerName="(?P<name>.+)" PlayerID=(-1|\d+) ProfileID=(?P<guid>\d+) Hash=(?P<hhash>.*)$'))
    ...
    def kick(self, client, reason='', admin=None, silent=False, *kwargs):
        ...
        self.queueEvent(
            b311.events.Event(self.getEventID('EVT_CLIENT_KICK'), {'reason': reason, 'admin': None}, client))
        client.disconnect()
