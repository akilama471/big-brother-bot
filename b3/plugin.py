...
from b311 import __version__ as b3_version
...
class Plugin(object):
    ...
    def __init__(self, console, config=None):
        ...
        self.eventmanager = b311.events.eventManager
        self.eventmap = dict()
        if isinstance(config, b311.config.XmlConfigParser) or isinstance(config, b311.config.CfgConfigParser):
            ...
            if config is not None:
                ...
                except b311.config.ConfigFileNotValid as e:
    ...
    def enable(self):
        ...
        """
        self._enabled = True
        name = b311.functions.right_cut(self.__class__.__name__, 'Plugin').lower()
    ...
    def loadConfig(self, filename=None):
        ...
        if filename:
            ...
            try:
                self.config = b311.config.load(filename)
            except b311.config.ConfigFileNotFound:
            ...
            self.config = b311.config.load(self.config.fileName)
        else:
            if self.requiresConfigFile:
    ...
    def getSetting(self, section, option, value_type=b311.STRING, default=None, validate=None):
        """
        ...
        def _get_level(value):
            """convert the given value to a b311 group level"""
            self.verbose('trying to convert value to b311 group level : %s', value)
            return self.console.getGroupLevel(str(value).lower().strip())

        def _get_duration(value):
            """convert the given value using b311.functions.time2minutes"""
            self.verbose('trying to convert value to time duration : %s', value)
            return b311.functions.time2minutes(str(value).strip())

        def _get_path(value):
            """convert the given path using b311.getAbsolutePath"""
            self.verbose('trying to convert value to absolute path : %s', value)
            return b311.getAbsolutePath(str(value), decode=True)

        def _get_template(value):
            """process the given value using b311.functions.vars2printf"""
            return b311.functions.vars2printf(value).strip()
        ...
        handlers = {
            b311.STRING: _get_string,
            b311.INTEGER: _get_integer,
            b311.BOOLEAN: _get_boolean,
            b311.FLOAT: _get_float,
            b311.LEVEL: _get_level,
            b311.DURATION: _get_duration,
            b311.PATH: _get_path,
            b311.TEMPLATE: _get_template,
            b311.LIST: _get_list,
        }
        ...
        try:
            val = self.config.get(section, option, value_type == b311.TEMPLATE)
        except b311.config.NoOptionError:
        ...
        try:
            ...
            try:
                ...
                if msg in self._default_messages:
                    self._messages[msg] = b311.functions.vars2printf(self._default_messages[msg]).strip()
                else:
                    raise
            _msg = self._messages[msg]
