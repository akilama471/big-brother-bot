...
import b311
from b311.storage.common import DatabaseStorage
        ...
        if time() - self._lastConnectAttempt < self._reconnectDelay:
            ...
            try:
                ...
                if not self.getTables():
                    ...
                    try:
                        self.console.info(
                            "Missing PostgreSQL database tables: importing SQL file: %s..." % b311.getAbsolutePath(
                                "@b311/sql/postgresql/b311.sql"))
                        self.queryFromFile("@b311/sql/postgresql/b311.sql")
                        ...
                        self.console.critical(
                            ...
                            (b311.getAbsolutePath("@b311/sql/postgresql/b311.sql"), e))
