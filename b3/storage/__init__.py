...
from b311.storage.mysql import MysqlStorage
from b311.storage.postgresql import PostgresqlStorage
...
def getStorage(dsn, dsnDict, console):
    ...
    if not dsnDict['protocol'] in PROTOCOLS:
        raise AttributeError('invalid storage protocol specified: %s: supported storage protocols are: %s' % (
        dsnDict['protocol'], ','.join(PROTOCOLS)))
