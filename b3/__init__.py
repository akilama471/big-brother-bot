...
version = 'AKARI SQUAD (B311) Developed Edition by KilerZack %s' % versionId

confdir = None
console = None

# STRINGS
B3_TITLE = 'AKARI SQUAD (B311) Developed Edition by KilerZack %s' % versionId
B3_TITLE_SHORT = 'AKR-B311 %s' % versionId
B3_COPYRIGHT = 'Copyright © 2021'
...
LEVEL = 5  # b311.clients.Group level
DURATION = 6  # b311.functions.time2minutes conversion
PATH = 7  # b311.getAbsolutePath path conversion
TEMPLATE = 8  # b311.functions.vars2printf conversion
...
def gethomepath():
    ...
    """
    path = os.path.normpath(os.path.expanduser('~/.b311')).encode().decode(sys.getfilesystemencoding())

    # RENAME v1.10.1 -> v1.10.7
    path_1 = os.path.normpath(os.path.expanduser('~/BigBrotherBot')).encode().decode(sys.getfilesystemencoding())
...
def getB3Path(decode=False):
    ...
    return decode(os.path.normpath(os.path.expanduser(path)))
...
def getConfPath(decode=False, conf=None):
    ...
    if not decode:
        return path
    return path.encode().decode(path)
...
def getAbsolutePath(path, decode=False, conf=None):
    ...
    :param conf: the current configuration being used :type XmlConfigParser|CfgConfigParser|MainConfig|str:
    """
    if path[0:4] == '@b311\\' or path[0:4] == '@b311/':
...
def getShortPath(filepath, decode=False, first_time=True):
    ...
    Will replace path string with proper tokens (such as @b311, @conf, ~, ...)
    :param filepath: the path to convert
    :param decode: if True will decode the path string using the default file system encoding before returning it
    :param first_time: whether this is the first function call attempt or not
    :return: string
    """
    # NOTE: make sure to have os.path.sep at the end otherwise also files starting with 'b311' will be matched
    ...
    b3path = getAbsolutePath('@b311/', decode) + os.path.sep
    if filepath.startswith(b3path):
        return filepath.replace(b3path, '@b311' + os.path.sep)
...
def start(mainconfig, options):
    """
    Main B3 startup.
    :param mainconfig: The B3 configuration file instance :type: b311.config.MainConfig
    ...
    sys.stdout.write('Starting Akari Squad B311     : %s\n' % getB3versionString())
    ...
    parsertype = mainconfig.get('b311', 'parser')
...
from b311.config import Xmlconfigparser, Cfgconfigparser, MainConfig
from b311.functions import clearscreen
from b311.functions import main_is_frozen
from b311.functions import right_cut
from b311.update import checkUpdate
