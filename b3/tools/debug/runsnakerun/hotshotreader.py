...
class FunctionRecord(object):
    ...
    recursive = property(get_recursive, None, None,
                         """Calls to the function where the function is already on the call stack""")
...
def loadHotshot(filename, yieldCount=10000):
    ...
    for i, (what, tdelta, fileno, lineno) in enumerate(reader):
        ...
        if givesDelta(what):
            key = fileno, lineno
            print
            'lineno', lineno, tdelta, getattr(getFunction(key), 'name', '')
            if what == whatEnter:
                key = (fileno, lineno)
...
def loadHotshot2(filename):
    ...
    for i, (what, tdelta, fileno, lineno) in enumerate(reader):
        if givesDelta(what):
            ...
            print
            'line time', (tdelta, fileno, lineno)
        else:
            print
            'unrecognised what', what
            for name in [n for n in dir(_hotshot) if n.startswith('WHAT_')]:
