...
#    * change: paths to b311/lib are now generalized via sys module
...
sys.path.append(os.path.join(pathname, 'b311', 'lib'))
...
import \
    cPickle as pickle  # raw matrix and raw stats exporter - cPickle is a C implementation that is more efficient than pickle
        ...
        for game_log in args:
            ...
            try:
                ...
                for line in self.file:
                    ...
                    m = re.match(f, line)
                    if m:
                        gametime = int(m.group('seconds')) + (
                                    int(m.group('minutes')) * 60)  # game time at the time this line was outputted
                        if self.debug: print('%i- gametime %s %s:%s' % (
                        i, str(gametime), str(m.group('minutes')), str(m.group('seconds'))))
                    ...
                    if previouscursor != currpos:
                        if self.faster:
                            if (i % 1000) == 0: print('Processing %s%% (byte %s of %s)...' % (
                            str(currpos * 100 / filesize), str(currpos), str(filesize)))
                        elif not self.morefaster:
                            print('Processing %s%% (byte %s of %s)...' % (
                            str(currpos * 100 / filesize), str(currpos), str(filesize)))
    ...
    def stats_per_second(self, *args):
        ...
        for game_log, matrix in args:
            ...
            stats['itemfreq'] = mstats.itemfreq(
                matrix)  # frequency of each item (each item being the count of the occurrencies for each number of lines per second)
            ...
            stats['itemscore'] = [(percentile, cstats.valueforpercentile(matrix, percentile)) for percentile in
                                  scorepercentiles]
            stats['skew'] = mstats.skew(
                matrix)  # if positive, there are more smaller than higher values from the mean. If negative, there are more higher than smaller values from the mean.
            if stats['skew'] > 0:
    ...
    def weighted_mean_merge(self, *args):
        ...
        for stats in args:
            ...
            try:
                ...
                print(
                    'Notice: one of the loaded stat file seems to have been generated by an older version of the Diagnostic tool ! You may miss some important new parameters !')
                pass
            for key, value in stats.iteritems():
                if key is not 'count' and key is not 'skewmeaning' and key is not 'cumfreq':
                    ...
                    try:  # doing the sum of all the stats (one stat at a time)
                        ...
                        superstats[key] = newstat[key]
        divisor = sum([stat['count'] for stat in
                       args])  # calculating the common divisor, being the sum of all the counts of lines of all the stats
        weighted_stats = dict((map(lambda (key, value): (key, float(value) / divisor),
                                   superstats.iteritems())))  # dividing each of the superstats by the common divisor, this gives us the final weighted_stats
...
if __name__ == '__main__':
    ...
    somestats = p.load_data(False,
                            r'/some/dir/somestats.txt')  # load_data(merge, files) # you can specify if you want to merge the stats or matrix right now to save a lot of cpu
