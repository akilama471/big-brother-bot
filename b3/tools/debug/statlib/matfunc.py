...
if __name__ == '__main__':
    ...
    'C.sum()', C.sum()
    print
    'C.map(math.cos)', C.map(cmath.cos)
    print
    ...
    print
    'Func coeffs:', funfit(points, basis)
    print
    'Poly coeffs:', polyfit(points, degree=5)
    points = (xvec, yvec) = funToVec(lambda x: math.sin(x) + 2 * math.cos(.7 * x + .1), low=0, high=3)
    print
    'Rational coeffs:', ratfit(points)
    ...
    A.eigs()
    print
    'Should be:', Vec([11.6158, 5.0000, -3.6158])
    print
    'det(A)'
    print
    A.det()
    ...
    c.eigs()
    print
    'Should be:', Vec([-8.9554, 43.2497, -19.2943])
    ...
    A.eigs()
    print
    'Should be:', Vec([3.5736, 0.1765, 11.1055, -3.8556])
    ...
    u, v = A.qr()
    print
    q is u  # Verify old result not used
    print
    r is v
    print
    u.mmul(v) == A  # Verify new result

    print
    'Test qr on 3x5 matrix'
    a = rand(3, 5)
    q, r = a.qr()
    print
    q.mmul(r) == a
