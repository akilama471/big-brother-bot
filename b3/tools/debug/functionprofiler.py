...
from b311.tools.debug.kthread import *
from b311.tools.debug.profilebrowser import *
...
def runprofile(mainfunction, output, timeout=60):
    ...
    b3main = KThread(
        target=profileb3)  # we open b311 main function with the profiler, in a special killable thread (see below why)
    print('Will now run the profiling and terminate it in %s seconds. Results will be saved in %s' % (
    str(timeout), str(output)))
    print('\nCountdown:')
    for i in range(0, 5):
        print(str(5 - i))
        time.sleep(1)
    ...
    time.sleep(float(
        timeout))  # after this amount of seconds, the b311 main function gets killed and the profiler will end its job
...
def runprofilesubprocess(mainfunction, output, timeout=60):
    ...
    try:
        ...
        # profileb3("""b311.tools.profile.subb3()""", output)
