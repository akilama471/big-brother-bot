...
ConfigFileNotFound = b311.exceptions.ConfigFileNotFound
ConfigFileNotValid = b311.exceptions.ConfigFileNotValid
NoOptionError = b311.exceptions.NoOptionError
NoSectionError = b311.exceptions.NoSectionError
...
class B3configparserMixin(object):
    ...
    def getDuration(self, section, setting=None):
        ...
        return b311.functions.time2minutes(value)
    ...
    def getpath(self, section, setting):
        ...
        :param setting: The configuration file setting.
        """
        return b311.getAbsolutePath(self.get(section, setting), decode=True)
    ...
    def getTextTemplate(self, section, setting=None, **kwargs):
        ...
        """
        value = b311.functions.vars2printf(self.get(section, setting, True)).strip()
        if len(kwargs):
            return value % kwargs
        return value
    ...
    def _write_item(fp, key, value):
        if (key.startswith(';') or key.startswith('#')) and value is None:
            ...
            for line in key.split('\n'):
                line = b311.functions.left_cut(line, ';')
                line = b311.functions.left_cut(line, '#')
                fp.write("; %s\n" % (line.strip(),))
        else:
...
def load(filename):
    ...
    if os.path.splitext(filename)[1].lower() == '.xml':
        ...
        # allow the use of empty keys to support the new b311.ini configuration file
        config = Cfgconfigparser(allow_no_value=True)

    filename = b311.getAbsolutePath(filename, True)
...
class MainConfig(B3configparserMixin):
    ...
    def _init_plugins_from_cfg(self):
        ...
        try:
            disabled_plugins_raw = self._config_parser.get('b311', 'disabled_plugins')
    ...
    def get_external_plugins_dir(self):
        ...
        if isinstance(self._config_parser, Xmlconfigparser):
            ...
            return self._config_parser.getpath("b311", "external_plugins_dir")
        else:
            raise NotImplementedError("unexpected config type: %r" % self._config_parser.__class__)
    ...
    def analyze(self):
        ...
        _mandatory_option('b311', 'parser')
        _mandatory_option('b311', 'database')
        _mandatory_option('b311', 'bot_name')

        ## PARSER CHECK
        if self.has_option('b311', 'parser'):
            try:
                b311.functions.getModule('b311.parsers.%s' % self.get('b311', 'parser'))
            except ImportError:
                analysis.append('invalid parser specified in b311::parser (%s)' % self.get('b311', 'parser'))

        ## DSN DICT
        if self.has_option('b311', 'database'):
            dsndict = b311.functions.splitDSN(self.get('b311', 'database'))
            if not dsndict:
                analysis.append(
                    'invalid database source name specified in b311::database (%s)' % self.get('b311', 'database'))
            elif dsndict['protocol'] not in b311.storage.PROTOCOLS:
                analysis.append('invalid storage protocol specified in b311::database (%s) : '
                                'valid protocols are : %s' % (dsndict['protocol'], ', '.join(b311.storage.PROTOCOLS)))
